(function () {
  ...
  function observeDynamicContent(callback, doc) {
    doc = doc || document;
    var timeout = null;
    var observer = new MutationObserver(function(mutations) {
      var hasSignificantChange = mutations.some(function(m) {
        return (
          ...
          (m.type === 'characterData' && m.target.innerText.trim().length > 50)
        );
      });
  ...
  async function scrapeUrl(url) {
    ...
    return new Promise(function(resolve) {
      chrome.runtime.sendMessage({ type: 'openTab', url: url }, function(response) {
        if (chrome.runtime.lastError || !response || !response.tabId) {
          console.error('Failed to open tab:', chrome.runtime.lastError ? chrome.runtime.lastError.message : 'No response');
          resolve(null);
          return;
        }
        ...
        chrome.tabs.onUpdated.addListener(function listener(tabIdUpdated, changeInfo) {
          if (tabIdUpdated === tabId && changeInfo.status === 'complete') {
            ...
            chrome.scripting.executeScript({
              target: { tabId: tabId },
              func: function(url) {
                var content = window.scraper.extract();
                chrome.runtime.sendMessage({ type: 'extracted', content: content, url: url });
              },
              args: [url]
            });
      ...
      chrome.runtime.onMessage.addListener(function handler(message) {
        if (message.type === 'extracted' && message.url === url) {
  ...
  function initializeExtension() {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function() {
        console.log('Scraper initialized for browser extension.');
      });
    } else {
      console.log('Scraper initialized for browser extension.');
    }
    ...
    window.addEventListener('popstate', function() {
      ...
      setTimeout(function() {
        ...
        if (content) {
          window.dispatchEvent(new CustomEvent('scraperContent', { detail: content }));
        }
      }, 500);
    });

    window.addEventListener('hashchange', function() {
      console.log('Hash change detected, re-extracting...');
      setTimeout(function() {
        var content = extractContent();
        if (content) {
          window.dispatchEvent(new CustomEvent('scraperContent', { detail: content }));
