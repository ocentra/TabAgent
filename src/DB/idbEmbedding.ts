...
export class Embedding extends BaseCRUD<Embedding> {
  ...
  static async create(
    ...
    assertDbWorker(dbWorker, Embedding.create.name, Embedding.name);
  ...
  static async get(id: string, dbWorker: Worker): Promise<Embedding | undefined> {
    assertDbWorker(dbWorker, Embedding.get.name, Embedding.name);
    const requestId = crypto.randomUUID();
  ...
  static async getByInputAndModel(input: string, model: string, dbWorker: Worker): Promise<Embedding | undefined> {
    assertDbWorker(dbWorker, Embedding.getByInputAndModel.name, Embedding.name);
    const requestId = crypto.randomUUID();
    return new Promise<Embedding | undefined>((resolve, reject) => {
        const handleMessage = (event: MessageEvent) => {
            if (event.data && event.data.requestId === requestId) {
  ...
  static async generateVectorWithModelWorker(input: string, model: string, modelWorker: Worker): Promise<ArrayBuffer> {
    assertDbWorker(modelWorker, Embedding.generateVectorWithModelWorker.name, Embedding.name);
    const requestId = crypto.randomUUID();
    return new Promise<ArrayBuffer>((resolve, reject) => {
      const handleModelResponse = (event: MessageEvent) => {
        if (event.data && event.data.requestId === requestId) {
          modelWorker.removeEventListener(MESSAGE_EVENT, handleModelResponse);
      ...
      modelWorker.addEventListener(MESSAGE_EVENT, handleModelResponse);
  ...
  toJSON() {
    return {
      __type: DB_ENTITY_TYPES.Embedding,
      id: this.id,
      input: this.input,
      vector: this.vector,
      model: this.model,
  ...
  static fromJSON(obj: any, dbWorker: Worker): Embedding {
    if (!obj) throw new Error('Cannot hydrate Embedding from null/undefined');
