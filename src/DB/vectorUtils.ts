 ...
 * @returns {number[]}
 */
export function parseEmbedding(embedding: any): number[] {
    if (!embedding) return [];
    ...
    if (typeof embedding === 'string') {
        try {
            ...
            // Swallow JSON parse errors: return empty array if parsing fails
        }
    }
    return [];
}
...
export function findNearestNeighbors(
    ...
    messages: Array<{embedding: any; [key: string]: any}>,
    topN: number = 5,
    metric: 'cosine' | 'euclidean' = 'cosine'
): Array<{score: number; message: any}> {
