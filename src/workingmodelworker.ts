...
const LOG_TRANSFORMERS_SETTINGS = false; // Enable settings comparison logging
 ...
 */
async function getChunkInfo(modelId: string, fileName: string): Promise<{ isChunked: boolean; totalChunks?: number; totalSize?: number }> {
    try {
...
/**
 * Fetch a single chunk from IndexedDB (CSP-safe version)
 */
...
/**
 * Assemble chunks into a complete file (CSP-safe version)
 */
async function assembleChunks(modelId: string, fileName: string, totalChunks: number, totalSize: number): Promise<ArrayBuffer> {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        if (currentOffset + chunkUint8Array.length > totalSize) {
            throw new Error(`Chunk ${i} would overflow buffer for ${fileName}`);
        }
 ...
 */
async function createStreamingResponseFromChunks(modelId: string, fileName: string, totalChunks: number, totalSize: number): Promise<Response> {
        ...
        async start(controller) {
            try {
                for (let i = 0; i < totalChunks; i++) {
                    ...
                    // Enqueue the chunk data directly
                    controller.enqueue(new Uint8Array(chunkArrayBuffer));
                    
                    if (i % 20 === 0 || i === totalChunks - 1) {
                        if (LOG_GENERAL) console.log(prefix, `[createStreamingResponseFromChunks] Streamed chunk ${i}/${totalChunks-1}`);
                    }
                }
 ...
 */
async function saveChunkedFileSafe(resourceUrl: string, blob: Blob, originalInput: RequestInfo | URL): Promise<void> {
    ...
    if (LOG_GENERAL) console.log(prefix, `[saveChunkedFileSafe] Streaming chunking ${fileName}: ${fileSize} bytes into ${totalChunks} chunks`);
    ...
    try {
        while (true) {
            ...
            // Save chunk directly without additional blob creation
            await saveToIndexedDB(chunkKey, new Blob([chunkData], { type: 'application/octet-stream' }));
            
            totalBytesProcessed += chunkData.length;
            chunkIndex++;
            
            if (chunkIndex % 10 === 0 || chunkIndex === totalChunks) {
                if (LOG_GENERAL) console.log(prefix, `[saveChunkedFileSafe] Streamed chunk ${chunkIndex}/${totalChunks} (${chunkData.length} bytes, total: ${totalBytesProcessed}/${fileSize})`);
            }
        }
        
        if (LOG_GENERAL) console.log(prefix, `[saveChunkedFileSafe] ✅ Successfully streamed ${fileName}: ${chunkIndex} chunks saved`);
...
async function tryServeFromIndexedDB(resourceUrl: string): Promise<Response | null> {
    ...
    try {
        ...
        // First check if file is stored as chunks
        const fileName = resourceUrl.split('/').pop() || '';
        const modelId = currentModelRepoId;
        ...
        if (modelId) {
            ...
            if (chunkedInfo.isChunked && chunkedInfo.totalChunks && chunkedInfo.totalSize) {
                if (LOG_GENERAL) console.log(prefix, `[tryServeFromIndexedDB] File is chunked: ${chunkedInfo.totalChunks} chunks, ${chunkedInfo.totalSize} bytes`);
                
                try {
                    // For files over 100MB, use streaming to avoid RAM issues
                    if (chunkedInfo.totalSize > 100 * 1024 * 1024) {
                        if (LOG_GENERAL) console.log(prefix, `[tryServeFromIndexedDB] Large file detected (${chunkedInfo.totalSize} bytes), using streaming response`);
                        ...
                        if (LOG_GENERAL) console.log(prefix, `[tryServeFromIndexedDB] ✅ SERVING CHUNKED FILE: ${resourceUrl}, size: ${assembledBuffer.byteLength} bytes`);
...
async function fetchFromNetworkAndCache(input: RequestInfo | URL, resourceUrl: string, options?: RequestInit): Promise<Response> {
    ...
    if (shouldChunkFile(fileSize)) {
        if (LOG_GENERAL) console.log(prefix, `[fetchFromNetworkAndCache] Large file detected (${fileSize} bytes), will chunk: ${resourceUrl}`);
        
        try {
            await saveChunkedFileSafe(resourceUrl, blob, input);
        } catch (chunkError) {
