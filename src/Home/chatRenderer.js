...
import { showError } from './utils.js'; // For displaying errors during render

let chatBodyElement = null;

//  Stores the reference to the main chat container element.
...
//  Creates the HTML structure for a single message bubble based on its properties.
export function displayMessage(msg) {
    ...
    if (msg.metadata?.type === 'scrape_result') {
        ...
        if (msg.isLoading) {
            ...
            // User color - MODIFIED for transparency and theme adherence
            // bubbleDiv.classList.add('bg-blue-100', 'dark:bg-blue-800', 'text-blue-900', 'dark:text-blue-100'); 
            bubbleDiv.classList.add(
                'bg-blue-500/20',         // Light mode: Blue with 20% opacity
                'dark:bg-blue-600/40',    // Dark mode: Darker blue with 40% opacity
                'text-gray-900',          // Light mode text (adjust if needed)
                'dark:text-gray-100'       // Dark mode text (adjust if needed)
            );
...
//  Clears the chat area and displays the initial welcome message.
...
//  Fetches messages for a given session ID and renders them in the chat area.
