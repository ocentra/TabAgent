...
import { SiteMapperMessageTypes } from './events/eventNames';
...
declare global {
  interface Window {
    mapper?: {
      crawlSingle: (doc: Document, url: string) => any;
      mapSite: (startUrl: string) => Promise<any>;
      version: string;
    };
  }
}
...
(function () {
  ...
  const visitedUrls: Set<string> = new Set(); // Track visited URLs
  const allUrls: Set<string> = new Set(); // Track all unique URLs (internal and external)
  let crawlQueue: string[] = []; // Queue for pending URLs
  let navigationData: Record<string, any> = {}; // URL -> PageNavigation
  let sitemap: string[] = []; // List of unique URLs

  function cleanText(text: string): string {
    return text.replace(/\s+/g, ' ').trim();
  }

  function extractNavigationElements(doc: Document, baseUrl: string): any[] {
    doc = doc || document;
    const elements: any[] = [];
    const seenLinks = new Set<string>(); // Deduplicate links/buttons per page
  ...
  function crawlSinglePage(doc: Document, url: string): any {
    doc = doc || document;
    ...
    try {
      ...
      const crawlableLinks = navigationElements
        .filter((el: any) => el.type === 'link' && el.href)
        .map((el: any) => el.href)
        .filter((href: string) => {
          try {
  ...
  async function crawlRecursive(startUrl: string): Promise<void> {
    ...
    const result = await new Promise<any | null>(resolve => {
      browser.runtime.sendMessage({ type: SiteMapperMessageTypes.OPEN_TAB, url: startUrl }, (response: any) => {
        ...
        browser.tabs.onUpdated.addListener(function listener(tabIdUpdated: number, changeInfo: any) {
          if (tabIdUpdated === tabId && changeInfo.status === 'complete') {
            ...
            browser.scripting.executeScript({
              target: { tabId: tabId },
              func: (url: string) => {
                const content = (window as any).mapper.crawlSingle(document, url);
        ...
        browser.runtime.onMessage.addListener(function handler(message: any) {
  ...
  async function mapSite(startUrl: string): Promise<any> {
  ...
  if (typeof browser !== 'undefined' && browser.runtime && browser.runtime.onMessage) {
    browser.runtime.onMessage.addListener((message: any, sender: any, sendResponse: any) => {
      ...
      if (message.type === SiteMapperMessageTypes.OPEN_TAB) {
        browser.tabs.create({ url: message.url, active: false }, (tab: any) => {
          if (browser.runtime.lastError) {
