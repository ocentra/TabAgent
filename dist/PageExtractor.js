...
function isBoilerplate(element) {
    ...
    return (boilerplateTags.includes(tagName) ||
...
function extractSegments(element) {
    ...
    elements.forEach(function (el) {
        ...
        if (!text && !el.querySelector('img, video, table') && !['BUTTON', 'FIGURE', 'VIDEO', 'TABLE'].includes(el.tagName))
            return;
        ...
        if (['H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(el.tagName)) {
            ...
            segments.push({
                type: 'video',
                data: {
                    src: el.src || el.querySelector('source')?.getAttribute('src') || '',
                    type: (el.querySelector('source')?.getAttribute('type') || el.type) || '',
...
function filterLinks(links) {
    ...
    links.forEach(function (link) {
        ...
        if (!href || !text || href.startsWith('javascript:') || href === '#' || href.startsWith('mailto:'))
            return;
        ...
        try {
            ...
            if (restrictedDomains.some(function (domain) { return url.hostname.includes(domain); }))
                return;
            if (url.hostname.includes('ad') || url.pathname.includes('ad') || url.search.includes('ad'))
                return;
        }
        catch {
            return;
